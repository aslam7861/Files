...
#={ Given a (left) H-module M as abelian group with H-action by phi: H -> Aut(M)
  ...
  and sequences of embeddings M -> N and projections N -> M according to R.
    ...
    if (length(H) == length(G)) 
        return M, phi, [Id(G)], identity_map(M), identity_map(M)       # [ map< M->M | m:->m > ],  [ map< M->M | m:->m > ];
    end 
   ...
   #= //cosetAct := map< car<G, R> -> car<R, H> |
    ...
    images = GrpAbFinGenElem[]
    for g in GG
        #c = cosetAct[B2[k][2]][position_ali(g, GG)]
        for k in [position_ali(N[i], B1) for i in 1:length(gens(N))] 
            c = cosetAct[B2[k][2]][position_ali(g, GG)]
            push!(images, inc[c[1]]( phi(c[2])(B2[k][1]) ))
        end 
     end
    ...
    > >;=#
    psi = function(g)
        return map_from_func(x-> N([elem_to_seq(x)[i]*images[position_ali(g, GG)][i] for i = 1:length(elem_to_seq(x)) ]), N, N)
    end
      #=  function psii(g)
...
#for p in prm 
    ...
    list = [lp, mp, GP, psilp]
    ML, psiML, qML = compute_lpmult_mod(L,pl,psi,list,theta, m);
...
indMLi, psiIndMLi, RLi, kappaMLi, projMLi = inducedModule(U, [GG[1]], psiM, G);
diagL = let X = indMLi
...
push!(lst, Tuple([indMLi, psiIndMLi, diagL, c2]))
#push!(lst, Tuple([indML, psiIndML, diagL, c2]))
...
psiJL = MapFromFunc( g -> actJL[position_ali(g,GG)], G, Hecke.MapParent(JL,JL, "automorphism")) ;
#=function psiJL(g)
