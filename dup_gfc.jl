...
function random_ali(o::NfOrd)
    ...
    #  L = nf(o)
    n = degree(L)
    return sum([rand(1:5) * b[i] for i = 1:n])
    #return sum( [ r[i]*b[i] for i in 1:n])
end
#=
function elem_to_seq(x::Union{nf_elem, NfOrdElem})
    if typeof(x) == NfOrdElem    
        return [coeff(x.elem_in_nf,i) for i = 0:degree(parent(x))-1]
    else
        [coeff(x, i) for i = 0:degree(parent(x))-1]
    end
end
=#
...
function lattice_gen_theta(psi, P)
    ...
    v = valuation(theta, P)
    v1 = 1 + Int(floor(absolute_ramification_index(P) / (p - 1)))   # absolute_ram_index if over relative 
    v = max(0, v1 - v)
    theta = ol(theta * pi^v)
    return theta, v
end
...
function lattice_ali(P, p, psi)
    ...
    while !bool
        ...
        if bool
            M1 = transpose(matrix([coefficients(elem_in_nf(x)) for x in Gen]))
            M1i = M1^(-1)
            for j = v+1:300  #200 will come form index of Standard lattice of M
                m = j
                M2 = transpose(matrix([coefficients(x.elem_in_nf) for x in basis(P^j)]))
                den = denominator(M1i * M2)
                if !(isdivisible_by(den, p))
                    return theta, m
                    break
                end
            end
    ...
    #= M1 = transpose(matrix([ coefficients(elem_in_nf(x)) for x in Gen]))
     M1i = M1^(-1)
     for j in v+1:300  #200 will come form index of Standard lattice of M
         m = j
         M2 = transpose(matrix([coefficients(x.elem_in_nf) for x in basis(P^j)]))
         den = denominator(M1i*M2)
         if !(isdivisible_by(den, p))
...
function Sunit_group_action(L, prm, psi)
    ...
    for i = 1:length(G)
        push!(tup_act, (GG[i], hom(u, u, [mu \ (psi(GG[i])(mu(u[j]))) for j = 1:ngens(u)])))
...
function exp_truncated(x::Hecke.LocalFieldElem, n)
    return sum([x^i // factorial(big(i)) for i = 0:n])
end
...
function compute_lpmult_mod(L::AnticNumberField, P::NfOrdIdl, psi, list, theta::NfOrdElem, m)
    ...
    ZgenS = [S[i] for i = 1:ngens(S)]

    #    // G-Wirkung auf L_P^\times / X  als Matrizen
    #check from here
    #M = Vector{fmpz_mat}[];
    M = fmpz_mat[]
    for k = 1:length(HH) # [1..#HH] do
        g = HH[k]
        bilder = Vector[]
        # bilder =fmpz_mat[];
    ...
    X, qX = quo(Y, [order(ZgenS[i]) * Y[i+1] for i = 1:length(ZgenS)])
    ...
    function lp_X(x)
        qX(
            Y(
                append!(
                    [Valuation(x)],
                    (phi_l_S(ol(inc_lp \ (x // pil^(Valuation(x)))))).coeff,
                ),
            ),
        )
    end
    function X_lp(y)
        yy = (qX \ (y)).coeff
        rem_1st = Vector([yy[i] for i = 2:length(yy)])
...
function inducedModule(M::GrpAbFinGen, H, phi, G)#: RepSys := 0) -> GrpAb, Map, SeqEnum, SeqEnum, SeqEnum
    ...
    N, proj, inc =
        direct_product([M for i = 1:Int(length(G) // length(H))]..., task = :both)
    ...
    GG = [g for g in G]
    cosetAct = [
        [[[position_ali(r, R), h] for r in R, h in H if r * h == g * tau][1] for g in GG] for tau in R
    ]
    ...
    psii = MapFromFunc(
        g -> MapFromFunc(
            x -> sum([
                elem_to_seq(x)[i] * images[position_ali(g, GG)][i] for i = 1:ngens(N)
            ]),
            N,
            N,
        ),
        G,
        Hecke.MapParent(N, N, "mapping"),
    )
    psi = MapFromFunc(
        g -> hom(N, N, [psii(g)(a) for a in gens(N)]),
        G,
        Hecke.MapParent(N, N, "mapping"),
    )
...
function gfc(L)
    ...
    prm = prime_divisors(discriminant(OL))
    # push!(prm, p0)
    # S = NfOrdIdl[]
    #    for p in prm
    #      for y in  [x[1] for x in prime_decomposition(OL,p)]
    #        push!(S, y)
    #     end 
    #  end
    #sub = subfields(L)
    #@assert length(class_group(L)[1]) ==1
    @show "unit group and action:"
    @time U, mU, sigU = Sunit_group_action(L, prm, psiL) #sunit_group_fac_elem(S)
    GG = [x for x in G]
    CartPrd = Iterators.product(GG, GG)
    ...
    thetaAll = []
    for p in prm
        pl = prime_decomposition(OL, p)[1][1]
        pil = uniformizer(pl)
        if ramification_index(pl) == 1
            theta = one(OL)
            m = 0
        else
            theta, m = lattice_ali(pl, p, psi)
        ...
        list = [lp, mp, GP, psilp]
        @show "At fin place, abel grp"
        @time ML, psiML, qML = compute_lpmult_mod(L, pl, psi, list, theta, m + 5)
        @show "induced module:"
        @time indML, psiIndML, RL, kappaML, projML = inducedModule(ML, GP, psiML, G)
        #diagL = MapFromFunc(x -> sum([kappaML[i]( qML( mp( psiL(RL[i]^-1)( mU(x))))) for i = 1:length(kappaML)]), U, indML);
        diagL = let X = indML
            MapFromFunc(
                x -> sum([
                    kappaML[i](qML(mp(psiL(RL[i]^-1)(mU(x))))) for i = 1:length(kappaML)
                ]),
                U,
                indML,
            )
    ...
    function psiM(g)
        return hom(U, U, [U[i] for i = 1:ngens(U)])
    end
    ...
    @show "time of induced at inf place:"
    @time indMLi, psiIndMLi, RLi, kappaMLi, projMLi = inducedModule(U, [one(G)], psiM, G)
    @show "diagL at inf place"
    @time diagLi = let X = indMLi
        MapFromFunc(
            x -> sum([kappaMLi[i](psiU(RLi[i]^-1)(x)) for i = 1:length(kappaMLi)]),
            U,
            X,
        )
    end
    ...
    #push!(lst, Tuple([indML, psiIndML, diagL, c2]))
    @show "idele group:"
    @time JL, projJL, inclJL = direct_product([lst[i][1] for i = 1:length(lst)]..., task = :both)
    ...
    @show "action map on JL:"
    actJL = [
        MapFromFunc(
            x -> sum([inclJL[k](lst[k][2](h)(projJL[k](x))) for k = 1:length(lst)]),
            JL,
            JL,
        ) for h in G
    ]
    ...
    psiJLL = MapFromFunc(
        g -> actJL[position_ali(g, GG)],
        G,
        Hecke.MapParent(JL, JL, "automorphism"),
    )
    ########change  map from macfrom to hom for cohomology
    @time psiJL = MapFromFunc(
        g -> hom(JL, JL, [psiJLL(g)(a) for a in gens(JL)]),
        G,
        Hecke.MapParent(JL, JL, "mapping"),
    )
    ...
    @time embJL = MapFromFunc(x -> sum([inclJL[i](lst[i][3](x)) for i = 1:length(lst)]), U, JL)
    # embJL := map< US -> JL | x :-> &+([ x @ lst[i,3] @ inclJL[i] : i in [1..#lst]] ) >;
    @time B = [embJL(g) for g in gens(U)]
    @show "CL idele classes"
    @time CL, qCL = quo(JL, B)
    #=function psiCL(g)
        return inv(qCL)*psiJL(g)*qCL 
    end=#

    psiCLr =
        MapFromFunc(g -> inv(qCL) * psiJL(g^-1) * qCL, G, Hecke.MapParent(CL, CL, "aut"))
    #=function psiCLr(g) 
        return psiCL(g^-1)
    end=#
    ac = [hom(CL, CL, [psiCLr(g)(a) for a in gens(CL)]) for g in gens(G)]
    @show "cohomology module:"
    @time CohL = Main.GrpCoh.CohomologyModule(G, ac)

    # CohL = CohomologyModule(G, CL, psiCLr);
    return CohL, psiJL, embJL, lst
